...
//  Cross Roads
//
//  Created by richardleandro on 11/05/19.
...
class GameViewController: UIViewController {
    ...
    func setupScene(){
        ...
        sceneView.present(scene, with: .fade(withDuration: 0.5),
    ...
    func setupActions(){
        ...
        jumpForwardAction = SCNAction.group([turnFowardAction,jumpAction,moveForwardAction])
    ...
    func addActions(for trafficNode: TrafficNode){
        guard let driveAction = trafficNode.directionRight ? driveRightAction :
            driveLeftAction else{
                return
        }
...
extension GameViewController: SCNPhysicsContactDelegate{
    ...
    func physicsWorld(_ world: SCNPhysicsWorld, didBegin contact: SCNPhysicsContact) {
        ...
        switch mask {
        case PhysicsCategory.chiken | PhysicsCategory.vehicle:
            gameOver()
...
extension GameViewController{
    ...
    func checkBlocks(){
        if scene.physicsWorld.contactTest(with: collisionNode.front.physicsBody!,
                                          options: nil).isEmpty{
            frontBlocked = false
        }
        if scene.physicsWorld.contactTest(with: collisionNode.right.physicsBody!,
                                          options: nil).isEmpty{
            rightBlocked = false
        }
        if scene.physicsWorld.contactTest(with: collisionNode.right.physicsBody!,
